...
namespace libcommon {
    ...
    PollEvent::PollEvent():poll_event_(), user_data_(NULL) { memset(&poll_event_, 0, sizeof(poll_event_)); }
    ...
    {
        pollfd fd = {breaker_.BreakerFD(), POLLIN, 0};
        events_.push_back(fd);
    }
    ...
    bool SocketPoll::Consign(SocketPoll& _consignor, bool _recover) {
        ...
        if (_recover) {
            ...
            if (it->events == _consignor.events_[0].events) {
                LOGD("----- events ---- %d %d ", it->events, _consignor.events_[0].events);
            }
